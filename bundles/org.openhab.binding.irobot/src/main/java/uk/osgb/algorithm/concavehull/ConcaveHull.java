...
public class ConcaveHull {
    ...
    //
    /**
     * generate a circular list edge representation of a hull in the form of a CLOSED coordinate list. the order of the
     * coordinates should be CCW
     * 
     * @param hullCoords
     * @return
     */
     ...
     * @return a collection of geometry that form the concave hull of the input data (may contains linestring as
     *         degenerated segments)
     */
    public Collection<Geometry> getConcaveHullBFS(TriangleChecker triChecker, boolean allowMultiParts,
            boolean keepLineSeg) {
        if (triChecker != null) {
            // TreeSet<Coordinate> nodeSet = new TreeSet<Coordinate>(hullDT);
    ...
    /**
     * Concave hull with multiple parts and holes (going through all triangles and use union operators to generate
     * results)
     * 
    ...
    class DLCirList<T> {
        ...
        /**
         * split the current circular list into two by link n1 and n2; current list contains edge n1-n2 and new list
         * containing edge n2-n1 will be returned
         * 
         * @param n1
         * @param n2
         * @return
     ...
     * System.out.println("geberating random data...");
     * 
     ...
     * double radMaxSQ = sf*sf*0.25;
     * double radMinSQ = (sf-thickness)*(sf-thickness)*0.25;
     * //
     * int numPts = 500;
     * Coordinate[] coords = new Coordinate[2*numPts];
     * double x, y;
     * double rSQ;
     * for(int i = 0; i < numPts; ++i) {
     * do{
     * x = rand.nextDouble()*sf;
     * y = rand.nextDouble()*sf;
     * rSQ = (x-cx)*(x-cx) + (y-cy)*(y-cy);
     * }while(rSQ < radMinSQ || rSQ > radMaxSQ);
     * coords[i] = new Coordinate(x, y);
     * }
     * 
     * sf = 500.0;
     * radMaxSQ = sf*sf*0.25;
     * radMinSQ = (sf-thickness)*(sf-thickness)*0.25;
     * 
     * for(int i = numPts; i < 2*numPts; ++i) {
     * do{
     * x = rand.nextDouble()*sf + sf*0.5;
     * y = rand.nextDouble()*sf + sf*0.5;
     * rSQ = (x-cx)*(x-cx) + (y-cy)*(y-cy);
     * }while(rSQ < radMinSQ || rSQ > radMaxSQ);
     * coords[i] = new Coordinate(x, y);
     * }
     ...
     * System.out.println(mp.convexHull().toText());
     * 
     * List<Geometry> geomCol = new LinkedList<Geometry>();
     * geomCol.add(mp);
     * TriCheckerAlpha checkerAlpha = new TriCheckerAlpha(100);
     ...
     * Geometry h = null;
     * 
     * System.out.println("--- Chi --- " + checkerChi.length);
     * hulls = cah.getConcaveHullBFS(checkerChi, true, false);
     * for(Geometry hull:hulls) {
     * System.out.println(hull.toText());
     * }
     * 
     * System.out.println("--- alpha---" + checkerAlpha.getR());
     * hulls = cah.getConcaveHullBFS(checkerAlpha, true, false);
     * for(Geometry hull:hulls) {
     * System.out.println(hull.toText());
     * }
     * //
     * System.out.println("-- alpha with hole----");
     * h = cah.getConcaveHullWithHolesAlpha(checkerAlpha);
     * System.out.println(h.toText());
     * 
     * System.out.println("-- chi wiht hole ----");
     * h = cah.getConcaveHullWithHolesChi(checkerChi);
     * System.out.println(h.toText());
